...
import com.vts.elendservice.config.KafkaProperties;
...
class ElendServiceKafkaResourceIT {
    ...
    void setup() {
        ...
        ElendServiceKafkaResource kafkaResource = new ElendServiceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/elend-service-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/elend-service-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
